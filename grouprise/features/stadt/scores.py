...
class Gestalt:
    ...
    def score(cls, instance):
        minimum_timestamp = django.utils.timezone.now() - datetime.timedelta(
            days=CORE_SETTINGS.SCORE_CONTENT_AGE
        )
        s = 0
...
class Group:
    ...
    def score(cls, instance):
        ...
        if isinstance(instance, groups.Group):
            ...
            for only_public, num_gestalten in (
                (False, num_members),
                (True, num_subscribers),
            ):
                ps = 0
                ...
                if only_public:
                    group_versions = group_versions.filter(
                        content__associations__public=True
                    )
                    group_comments = group_comments.filter(
                        content__associations__public=True
                    )
                    group_conversations = []
                else:
