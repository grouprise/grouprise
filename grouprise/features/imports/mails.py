...
MAGIC_SUBJECT_FOR_INTERNAL_ERROR_TEST = (
    "ijoo9zee7Cheisoochae2Ophie4ohx9ahyai3ux1Quae0Phu"
)
...
ParsedMailAttachment = collections.namedtuple(
    "ParsedMailAttachment", ("content_type", "filename", "data", "model_obj")
)
...
class ParsedMailMessage(
    ...
    def from_email_object(cls, email_obj, from_address=None):
        ...
        return cls(
            email_obj.get("Subject") or cls.MISSING_SUBJECT_DEFAULT,
            text_content,
            email_obj.get_all("To"),
            from_address,
            email_obj,
            email_obj.get("Message-ID"),
            cls.parse_date(email_obj.get("Date")),
            attachments,
        )
...
class ContributionMailProcessor:
    ...
    def __init__(self, default_reply_to_address=None, response_from_address=None):
        ...
        self.auth_token_regex = re.compile(
            r"^{prefix}([^@]+){suffix}$".format(
    ...
    def _create_contribution(self, message, container, gestalt, in_reply_to=None):
        ...
        files.File.objects.create_from_message_attachments(
            message.attachments, attached_to=contribution
        )
        return contribution
    ...
    def _process_authenticated_reply(self, message, auth_token_text):
        ...
        contribution = self._create_contribution(
            message, container, auth_token.gestalt, in_reply_to=in_reply_to_text
        )
    ...
    def _process_initial_thread_contribution(self, message, recipient):
        ...
        if error_text is not None:
            logger.error(
                'Could not process receiver "%s" in message %s. %s',
                recipient,
                message.id,
                error_text,
            )
        ...
        if gestalt and gestalt.user.has_perm(
            ...
            conversation = conversations.Conversation.objects.create(
            ...
            associations.Association.objects.create(
                ...
                entity_id=group.id,
                container_type=conversation.content_type,
                container_id=conversation.id,
            )
            post_create.send(sender=None, instance=contribution)
        else:
            logger.warning(
                "Rejected message (%s) for <%s>: sender lacks permission",
                message.id,
                recipient,
            )
            raise MailProcessingFailure(
    ...
    def process_message_for_recipient(self, message, recipient):
        ...
        if message.subject == MAGIC_SUBJECT_FOR_INTERNAL_ERROR_TEST:
            # for unittests
            raise ValueError(
                "Magic Test Subject discovered: {}".format(
    ...
    def send_error_mail_response(
        self, message, error_message, recipient=None, fail_silently=False
    ):
