...
class Guest(images.ImageMixin, memberships.MemberMixin, Test):
    ...
    def create_group_gallery(self, **kwargs):
        self.client.force_login(self.gestalt.user)
        kwargs.update(
            {"title": "Group Gallery", "text": "Test", "images": [self.image.pk]}
        )
    ...
    def test_guest_internal_group_gallery(self):
        ...
        self.assertNotContainsLink(
            obj=self.group, link_url=self.get_group_gallery_url()
        )
...
class Gestalt(
    ...
    def create_group_gallery(self, **kwargs):
        ...
        return self.client.post(
            self.get_url("create-group-gallery", self.group.slug), kwargs
        )
    ...
    def test_gestalt_gallery_link(self):
        ...
        self.assertContainsLink(
            self.client.get(self.group.get_absolute_url()),
            self.get_url("create-group-gallery", self.group.slug),
        )

    def test_gestalt_create_gallery(self):
        self.assertEqual(
            self.client.get(self.get_url("create-gallery")).status_code, 200
        )
