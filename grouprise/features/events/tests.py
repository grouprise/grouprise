...
class Guest(memberships.MemberMixin, grouprise.core.tests.Test):
    def create_event(self, **kwargs):
        ...
        self.client.post(
            self.get_url("create-event"), _get_adjusted_event_args(**kwargs)
        )
        self.client.logout()
    ...
    def create_group_event(self, **kwargs):
        self.client.force_login(self.gestalt.user)
        self.client.post(
            self.get_url("create-group-event", self.group.slug),
    ...
    def test_guest_public_event(self):
        ...
        self.assertContainsLink(
            self.client.get(self.gestalt.get_absolute_url()), event_url
        )
...
class Gestalt(memberships.AuthenticatedMemberMixin, grouprise.core.tests.Test):
    ...
    def test_gestalt_comment_event(self):
        ...
        self.assertRedirect(
            url=self.get_event_url("Gestalt Comment Event"),
            method="post",
            data={"text": "Comment"},
        )
...
class TwoGestalten(
    ...
    def create_event(self, **kwargs):
        ...
        self.client.post(
            self.get_url("create-group-event", self.group.slug), event_args
        )
        self.association = associations.Association.objects.get(
            content__title=event_args["title"]
        )
...
class GroupCalendarExportMember(
    ...
    def create_group_event(self, **kwargs):
        ...
        self.assertEqual(
            count_before + 1, count_after, "Failed to create item: {}".format(kwargs)
        )
...
class TestUrls(grouprise.core.tests.Test):
    def test_events_404(self):
        ...
        self.assertEqual(r.status_code, 404)
        r = self.client.get(
            django.urls.reverse("group-events-feed", args=["non-existent", "public"])
        )
