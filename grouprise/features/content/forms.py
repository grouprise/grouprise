...
class RepetitionPeriod(enum.Enum):
    ...
    @classmethod
    def _get_date_by_month_offset(self, base_date, month_offset_count, day_override=None):
    ...
    def get_repetition_datetimes(self, base_date, count):
        ...
        if self is self.ONCE:
            ...
            yield from (base_date + datetime.timedelta(weeks=index) for index in offsets)
            ...
            for index in offsets:
                ...
                if first_possible_day is not None:
                    ...
                    for day_offset in range(7):
                        candidate = first_possible_day + datetime.timedelta(days=day_offset)
                        if ((candidate.month == first_possible_day.month)
                                and (candidate.weekday() == wanted_weekday)):
                            yield candidate
                            break
...
class Create(forms.ModelForm):
    ...
    time_repetitions_period = forms.ChoiceField(
            ...
            choices=((RepetitionPeriod.ONCE.value, 'einmalig'),
    ...
    time_repetitions_count = forms.DecimalField(
            label='Anzahl', help_text='Anzahl der Ereignisse dieser Art.',
    ...
    def _create_time_based_repetitions(
            cls, times, container_class, model, entity_id, entity_type, title, text, author, image,
            place, duration, all_day, commit):
        ...
        for time in times:
            ...
            new_instance.container.versions.create(
    ...
    def save(self, commit=True):
        with django.db.transaction.atomic():
            ...
            if repetitions_count > 0:
                ...
                start_times = period.get_repetition_datetimes(first_time, repetitions_count)
                self._create_time_based_repetitions(
                    start_times, self.container_class, self._meta.model, self.instance.entity_id,
                    self.instance.entity_type, self.cleaned_data['title'],
                    self.cleaned_data['text'], self.author, self.cleaned_data.get('image'),
                    self.cleaned_data.get('place', ''),
                    self.cleaned_data.get('until_time') - first_time,
                    self.cleaned_data.get('all_day', False), commit)
            return result
