...
class CreateIsAllowed:
    def test_create(self):
        ...
        response = self.client.post(self.get_url("group-create"), {"name": "Test"})
        group = self.assertExists(models.Group, name="Test")
        self.assertRedirects(response, group.get_absolute_url())


class CreateAnonymous(HasSidebarAndGroupsLinks, CreateIsAllowed, tests.Test):
    """
...
class SubscriberGroup(AuthenticatedSubscriberMixin, TestCase):
    ...
    def test_recommendation(self):
        ...
        r = self.client.post(
            recommend_url,
            dict(
                recipients="foo @{}, <{}>".format(
                    self.gestalt.user.username, recommend_email
                ),
        ...
        self.assertEqual(
            set(recipients), set([self.gestalt.user.email, recommend_email])
        )
