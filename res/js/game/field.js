...
function Field(size, initial = 0) {
    ...
    function replace(list, val) {
        const current = get_filled(val, comparison.equality);
    ...
    function get_filled(value = initial, cmp = comparison.inequality) {
        const result = [];
    ...
    function empty() {
        while(true) {
            const rand_pos = [random(0, size - 1), random(0, size - 1)];

            if(get(rand_pos) === initial) {
                return rand_pos;
            }
        }
    }
    ...
    Object.assign(iface, { get, set, replace, get_filled, neighbor, empty });
    return iface;
}
